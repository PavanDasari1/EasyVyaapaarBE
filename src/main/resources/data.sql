-- ================================================================
-- EasyVyaapaar Authentic Seed Data
-- Only inserts if product doesn't exist (idempotent)
-- ================================================================
...
INSERT INTO products (name, category, unit, current_stock, minimum_stock, price)
SELECT 'Urad Dal', 'Pulses', 'KG', 12.000, 3.000, 140.00
...
INSERT INTO products (name, category, unit, current_stock, minimum_stock, price)
SELECT 'Maida', 'Grains', 'KG', 25.000, 5.000, 40.00
WHERE NOT EXISTS (SELECT 1 FROM products WHERE name = 'Maida');
...
INSERT INTO products (name, category, unit, current_stock, minimum_stock, price)
SELECT 'Detergent', 'Hygiene', 'PACKET', 20.000, 5.000, 110.00
WHERE NOT EXISTS (SELECT 1 FROM products WHERE name = 'Detergent');
...
INSERT INTO products (name, category, unit, current_stock, minimum_stock, price)
SELECT 'Groundnuts', 'Pulses', 'KG', 25.000, 5.000, 130.00
WHERE NOT EXISTS (SELECT 1 FROM products WHERE name = 'Groundnuts');

-- Sample transactions (initial history)
INSERT INTO inventory_transactions (product_id, operation, quantity, unit, source, original_voice_text, notes)
SELECT p.id, 'ADD', 5.000, 'BAG', 'VOICE', 'Rice 5 bags add cheyyi', 'Initial stock entry'
FROM products p WHERE p.name = 'Rice'
AND NOT EXISTS (SELECT 1 FROM inventory_transactions LIMIT 1);

INSERT INTO inventory_transactions (product_id, operation, quantity, unit, source, original_voice_text, notes)
SELECT p.id, 'REMOVE', 2.000, 'BAG', 'VOICE', 'Sugar 2 bags sold', 'Initial stock entry'
